...
- **Processo DHCP**:
  1. **DHCPDISCOVER**: l'host cerca un server DHCP inviando un messaggio di richiesta in broadcast.
  2. **DHCPOFFER**: i server DHCP rispondono proponendo un indirizzo IP.
  3. **DHCPREQUEST**: l'host accetta una delle offerte e richiede l'indirizzo IP.
  4. **DHCPACK**: il server DHCP conferma la configurazione con un messaggio di risposta.